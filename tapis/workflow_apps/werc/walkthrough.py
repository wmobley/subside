...
# ## 1. Setup
#
# Put the repo's ``subside/`` directory on ``sys.path`` so ``analysis``
...
#
# Read from the environment (or ``~/.netrc``) — never hardcoded. Export them
# before running:
#
#     export EARTHDATA_USERNAME=<earthdata-username>
#     export EARTHDATA_PASSWORD=<earthdata-password>

# %%
if os.environ.get("EARTHDATA_USERNAME") and os.environ.get("EARTHDATA_PASSWORD"):
    print("Using EARTHDATA_USERNAME from the environment.")
else:
    ...
    try:
        ...
        if creds:
            ...
            raise SystemExit(
                "No EARTHDATA_USERNAME / EARTHDATA_PASSWORD env vars and no "
                "urs.earthdata.nasa.gov entry in ~/.netrc. Set one before continuing "
                "(never hardcode credentials in this file)."
            )
    except FileNotFoundError:
        raise SystemExit(
            "No EARTHDATA_USERNAME / EARTHDATA_PASSWORD env vars and no ~/.netrc. "
            "Export the env vars before continuing (never hardcode credentials)."
        )
...
# %%
from analysis.werc.config import WercRunConfig
...
# %%
from analysis.h2i_lab.runner import run as h2i_run
...
# %%
from analysis.werc import stack as stack_mod
...
# %%
from analysis.werc import export
...
from analysis.werc.runner import run as werc_run
