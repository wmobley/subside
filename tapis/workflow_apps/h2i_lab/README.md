...
- Batch entrypoint: `tapis/workflow_apps/h2i_lab/run.sh`
- Tapis app definition draft: `tapis/workflow_apps/h2i_lab/app-cpu.json`
...
3. App runs `python -m analysis.h2i_lab.cli run`.
...
[`walkthrough.py`](walkthrough.py) in this directory exercises the same code path as `run.sh` end-to-end (config → preflight → download → preview → archive) against real Earthdata. Use it to validate environment + credentials before building the container or submitting a Tapis job.

Run end-to-end:

```bash
python tapis/workflow_apps/h2i_lab/walkthrough.py
```
...
3. Activates it and runs `python -m analysis.h2i_lab.cli run …`.
...
```bash
docker build -f tapis/workflow_apps/h2i_lab/Dockerfile -t subside-h2i-opera-analysis:dev .
```
...
```bash
mkdir -p .docker-work
docker volume create subside-conda-h2i
