...
Exercises the extracted ``analysis.h2i_lab`` package against real
...
#
# Read from the environment — never hardcoded. Export them before running
# (or run the cell after setting them in your shell / IDE run config):
#
#     export EARTHDATA_USERNAME=<earthdata-username>
#     export EARTHDATA_PASSWORD=<earthdata-password>
#
# A ``~/.netrc`` entry for ``urs.earthdata.nasa.gov`` also works for the
# underlying downloader if you prefer not to set env vars.

# %%
if not (os.environ.get("EARTHDATA_USERNAME") and os.environ.get("EARTHDATA_PASSWORD")):
    raise SystemExit(
        "Set EARTHDATA_USERNAME / EARTHDATA_PASSWORD in the environment before "
        "running (export them in your shell or IDE run config). Never hardcode "
...
# %%
from analysis.h2i_lab.runner import run
