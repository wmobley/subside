...
- `subside/analysis/{h2i_lab,werc}/` — the Python code the apps run.
...
Each Tapis app sets a `STAGE` env var (or first arg) that `run.sh` dispatches on; the container then invokes the right `analysis.<pkg>.cli <subcommand>`. App-level differences are queue / `maxMinutes` / `appArgs` / `fileInputs`, not the image. This keeps the GHA build down to two images while giving the pipeline N retry/skip points.
...
| --------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| `discover`            | `tapis_job` | `python -m analysis.h2i_lab.cli preflight`. Fast (~30 s). Outputs `preflight-manifest.json` with frame_ids, product_count, warnings. Fails pipeline if `product_count == 0`.                          | `subside-h2i-discover` (`STAGE=preflight`)        |
| `download-opera`         | `tapis_job` | `python -m analysis.h2i_lab.cli run`. Heavy: parallel Earthdata download + crop, preview, zip. Outputs `run-manifest.json` + NetCDFs + previews + archive.                                            | `subside-h2i-analyze` (`STAGE=run`)               |
| `build-stack`         | `tapis_job` | `python -m analysis.werc.cli build-stack`. Reads the per-product NetCDFs from `download-opera`'s archive; writes one combined `displacement_stack.nc` + `disp_products.json`.                            | `subside-werc-build-stack`                        |
| `compute-reference`   | `tapis_job` | `python -m analysis.werc.cli compute-reference`. Reads stack; auto- or manual-picks reference pixels; writes `displacement_stack_referenced.nc` + `reference_anchor_FRAME{ID}.json` + `reference_summary.json`. | `subside-werc-compute-reference`                  |
| `estimate-velocity`   | `tapis_job` | `python -m analysis.werc.cli estimate-velocity`. Reads referenced stack; linear fit; writes `velocity.nc` + `velocity_summary.json`.                                                                  | `subside-werc-estimate-velocity`                  |
| `export-geotiffs`     | `tapis_job` | `python -m analysis.werc.cli export-geotiffs`. Reads referenced stack + velocity; writes `opera_disp_s1_cumulative.tif` + `opera_disp_s1_velocity.tif` + `export_summary.json`.                       | `subside-werc-export-geotiffs`                    |
...
Independently of the Tapis Workflows wiring above, the Python code under `analysis/` was refactored to surface the cross-cutting primitives both packages need. A new `analysis/etl/` package holds dataset-agnostic helpers; `h2i_lab` and `werc` import from it. See [`analysis/REUSABLE_PRIMITIVES.md`](../analysis/REUSABLE_PRIMITIVES.md) for the function-by-function catalog and the deferred items.

What this means for the Tapis Workflows side:

- **No Dockerfile / app JSON changes.** The Tapis images already `COPY analysis /opt/subside/analysis`, so `etl/` ships with both images automatically.
...
- The new analysis stands up its own `analysis/<new_pkg>/{config,runner,cli}.py` composition layer.
- It imports `analysis.etl.{auth, aoi, manifest, stack, archive}` for the cross-cutting bits — Earthdata auth survives URS OAuth redirects, AOI loading + bbox conversions, manifest writing, NetCDF stack save/load, zip archive.
...
| ---------------------------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------- |
| Per-stage WERC CLI subcommands (`build-stack`, `compute-reference`, `estimate-velocity`, `export-geotiffs`) | done       | `analysis/werc/cli.py`, `analysis/werc/runner.py`                              |
...
| Color | Group |
| --- | --- |
| Blue | `analysis/etl/` — reusable primitives (any analysis) |
| Green | `analysis/h2i_lab/` — OPERA H2I package |
| Purple | `analysis/werc/` — OPERA WERC package |
