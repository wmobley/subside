...
- **WERC extraction landed.** `analysis/werc/` now contains `stack.py`, `reference.py` (auto + manual + per-frame anchor JSON persistence), `velocity.py`, `export.py`, `config.py` (`WercRunConfig` wraps `H2IRunConfig`), `runner.py` (composes `h2i_lab.runner.run` for the download stage), and `cli.py`. Function map mirrors h2i_lab's in [`analysis/werc/README.md`](analysis/werc/README.md).
- **Companion Tapis batch app.** `tapis/workflow_apps/werc/` scaffolded with `Dockerfile`, `run.sh`, `app-cpu.json` (`subside-werc-opera-analysis`), `environment.yaml`, `run-config.example.json`, `README.md`.
- **Cookbook-style runtime conda install.** Both Tapis apps switched from baked-env images to thin `ubuntu:22.04` + `run.sh` that downloads miniconda + materializes the env into `${ENV_INSTALL_DIR:-$WORK}/miniconda3` on first invocation and reuses thereafter. `UPDATE_CONDA_ENV=true` env var (exposed via `app-cpu.json`) forces a rebuild. Matches the `examples/notebookExamples/h2i_lab/run.sh` pattern.
...
- **Local validation.** `tapis/workflow_apps/{h2i_lab,werc}/walkthrough.py` cell-style scripts (`# %%` markers) exercise the full pipeline end-to-end or stepwise in any IDE. Shared `examples/sample_aoi.geojson` over Houston-Galveston.
- **Earthdata auth bug fix.** Plain `requests.Session.auth` was being stripped on the cumulus.asf → urs.earthdata OAuth redirect. New `analysis/h2i_lab/_session.py` introduces `EarthdataSession` (subclasses `requests.Session`, overrides `rebuild_auth` to preserve `Authorization` only for `urs.earthdata.nasa.gov`). Now used by `metadata.fetch_product_bytes` and `download.process_file`.
...
- [x] Use Tapis Workflows for orchestration and Tapis Jobs or a registered app/container for heavy OPERA analysis.
  ...
  - Resolved 2026-05-28: two Tapis apps (`subside-h2i-opera-analysis`, `subside-werc-opera-analysis`) scaffolded as containers under `tapis/workflow_apps/`. Orchestration layer (Tapis Workflows pipeline chaining the two) still open.
...
- [x] Inventory the two OPERA notebooks and map each cell to either analysis, visualization, auth, or dead/example code.
  ...
  - Resolved 2026-05-28: cell-by-cell tables in `analysis/h2i_lab/README.md` and `analysis/werc/README.md`.

- [ ] Complete H2I Lab extraction and harden the first Tapis batch app.
  - Explanation: Initial function map and scaffolding live in `analysis/h2i_lab/` and `tapis/workflow_apps/h2i_lab/`. Remaining work is dependency verification, credential handling, image build/push, and one real Tapis smoke test.
...
- [x] Create a reusable analysis package for OPERA DISP-S1 processing.
  - Explanation: Move analysis logic into normal Python modules with functions that can be tested and called from a CLI. Candidate package name: `analysis` or `subside_backend.analysis`.
  - Resolved 2026-05-28: `analysis/h2i_lab/` and `analysis/werc/` are importable, function-based, and reusable.
...
- [x] Build a CLI entrypoint for local and Tapis execution.
  ...
  - Resolved 2026-05-28: `python -m analysis.h2i_lab.cli {preflight|run}` and `python -m analysis.werc.cli {preflight|run}`, both accepting `--config` + `--output-dir`. Tapis `run.sh` invokes the same modules.
...
- [x] Decide how Earthdata credentials are stored and supplied to runs.
  ...
  - Resolved 2026-06-01: a dedicated **Earthdata service account** is the credential source (not personal logins). It lives server-side in `subside/.env` (`EARTHDATA_USERNAME`/`EARTHDATA_PASSWORD`, gitignored, documented in `.env.sample`); `api/manager._stage` builds a `.netrc` from it and stages it per-run via Tapis Files, so the secret never transits the API request body or browser-visible job args. Inline request creds remain accepted but discouraged. The previously leaked password (plaintext in the `walkthrough.py` scripts) has been **rotated**; follow-on hardening is to move those walkthrough scripts off any hardcoded value entirely.
...
- [x] Add a tiny AOI/date-range fixture for local testing.
  ...
  - Resolved 2026-05-28: `examples/sample_aoi.geojson` (Houston-Galveston 0.2°×0.2° box) + `tapis/workflow_apps/{h2i_lab,werc}/walkthrough.py` cell-style drivers that exercise the full pipeline against a 3-month window.
...
- [x] Session 1: Notebook inventory and extraction map.
  ...
  - Done 2026-05-28: h2i_lab + werc function maps in their READMEs; package layout `analysis/<pkg>/{config,stack,reference,velocity,export,runner,cli}.py`.
