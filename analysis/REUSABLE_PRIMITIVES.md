...
Scope chosen: pieces that align with both packages now. Five modules under `analysis/etl/`.
...
To avoid breaking any caller that imports `from analysis.h2i_lab.aoi import load_aoi`, `from analysis.h2i_lab.preview import archive_results`, or `from analysis.werc.stack import stack_epsg`, those modules now re-export the etl symbols. New code should import directly from `analysis.etl.*`.
...
- `from analysis.etl.auth import EarthdataSession, earthdata_credentials` — any URS-protected source.
- `from analysis.etl.aoi import bounds_from_aoi, load_aoi, bbox_dict_from_*` — any geospatial AOI flow.
- `from analysis.etl.manifest import write_json` — any pipeline manifest.
- `from analysis.etl.stack import save_stack, load_stack, stack_epsg` — any NetCDF time-series.
- `from analysis.etl.archive import archive_results` — any zip output.
